CREATE OR REPLACE FUNCTION query80(sales_date date)
...
-- データマート作成
--
DROP TABLE IF EXISTS datamart80 CASCADE;
CREATE TABLE datamart80 AS
SELECT day, channel, id, sales, returns, profit
  FROM (SELECT day FROM generated_days WHERE day BETWEEN DATE '1998-01-01' AND DATE '2002-08-31') AS T1,
       LATERAL (SELECT * FROM query80(CAST(T1.day AS date))) AS T2;
