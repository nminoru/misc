...
-- 
CREATE OR ALTER FUNCTION query05(@SALESDATE date)
RETURNS TABLE
AS
RETURN
(
...
WITH ssr AS (
    ...
    WHERE date_sk = d_date_sk
       AND d_date BETWEEN @SALESDATE AND dateadd(day, 14, @SALESDATE)
...
FROM
    day_series
    CROSS APPLY (SELECT * FROM query05(day_series.day)) body;
